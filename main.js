...
function appendToDisplay(value){
    if(currentDisplay === "0" || resultDisplay){
        currentDisplay = value; // If the current display is "0" or the result is already displayed, replace it with the new value
    }
    else{
        currentDisplay += value; // Otherwise, concatenate the new value to the current display
    }

    resultDisplay = false; // Reset the result display flag to false, as the user entered a new value

    updateDisplay(); // Update the calculator display to show the new content
}
